    ...
    <div className="w-full max-w-6xl mx-auto space-y-6 animate-fade-in pb-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-[var(--text-secondary)] mt-0.5">Immutable logs of script parsing, executions, downloads, and auth events.</p>
        </div>

        <div className="flex items-center gap-3">
          <button
            onClick={() => handleExport("csv")}
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#0f0f16] text-[var(--text-secondary)] hover:text-white transition-all text-xs font-semibold cursor-pointer"
          >
      ...
      <form onSubmit={handleApplyFilters} className="glass p-5 rounded-2xl border border-[rgba(255,255,255,0.06)] bg-[#0f0f16] space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
          {/* Username search */}
          <div>
            <label className="block text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wide mb-1.5">Username</label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-1.5 bg-[#0a0a0f] border border-[rgba(255,255,255,0.06)] rounded-xl text-xs text-white placeholder:text-[var(--text-muted)] focus:outline-none focus:border-[#fb4e0b]"
              />
            </div>
          </div>
          ...
          {/* Risk select */}
          <div>
            <label className="block text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wide mb-1.5">Risk Level</label>
            <select
              value={riskLevel}
          ...
          {/* Status select */}
          <div>
            <label className="block text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wide mb-1.5">Status</label>
            <select
              value={status}
          ...
          {/* Start Date */}
          <div>
            <label className="block text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wide mb-1.5">Date From</label>
            <input
              type="date"
              value={dateFrom}
          ...
          {/* End Date */}
          <div>
            <label className="block text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wide mb-1.5">Date To</label>
            <input
              type="date"
              value={dateTo}
      ...
      <div className="rounded-2xl border border-[rgba(255,255,255,0.06)] bg-[#0f0f16] overflow-hidden shadow-xl">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 gap-3">
            ...
            <span className="text-xs text-[var(--text-secondary)]">Retrieving audit trail...</span>
          </div>
        ) : items.length === 0 ? (
                ...
                {items.map((item) => {
                  ...
                  return (
                    <>
                      <tr 
                        key={item.id}
                        className={`border-b border-[rgba(255,255,255,0.04)] hover:bg-[rgba(255,255,255,0.01)] text-[var(--text-secondary)] cursor-pointer transition-all ${
                      ...
                      >
                        ...
                        <td className="p-4 truncate max-w-[120px]" title={item.filename || ""}>
                          {item.filename || <span className="text-[var(--text-muted)]">—</span>}
                        </td>
                      ...
                      {isExpanded && (
                        <tr className="border-b border-[rgba(255,255,255,0.04)] bg-[rgba(255,255,255,0.01)] text-xs text-[var(--text-secondary)]">
                            ...
                            {detailsLoad ? (
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in border-t border-[rgba(255,255,255,0.04)] pt-4">
                                ...
                                <div className="space-y-3">
                                  ...
                                  {item.script_hash && (
                                    <div className="flex justify-between border-b border-[rgba(255,255,255,0.03)] pb-1.5">
                                      <span className="text-[var(--text-muted)]">Script Hash (SHA256)</span>
                                  ...
                                  {item.summary && Object.keys(item.summary).length > 0 && (
                                    <div>
                                      <span className="text-[var(--text-muted)] block mb-1">Details Summary</span>
                                      <pre className="p-3 bg-[#0a0a0f] border border-[rgba(255,255,255,0.05)] rounded-xl font-mono text-[10px] text-[var(--text-secondary)] overflow-x-auto whitespace-pre-wrap leading-relaxed">
                                ...
                                <div className="space-y-3">
                                  ...
                                  {detail.files && detail.files.length > 0 ? (
                                    <div className="space-y-2">
                                      <span className="text-[var(--text-muted)] text-[11px] block">Generated output files:</span>
                                    ...
                                    {detail.llm_logs && detail.llm_logs.length > 0 ? (
                                      detail.llm_logs.map((log, idx) => (
                                        <div key={idx} className="grid grid-cols-1 md:grid-cols-2 gap-4 p-3 bg-[#0a0a0f] border border-[rgba(255,255,255,0.05)] rounded-xl">
                                          <div>
                                            ...
                                            <pre className="font-mono text-[10px] text-[var(--text-secondary)] whitespace-pre-wrap max-h-60 overflow-y-auto leading-relaxed">{log.prompt}</pre>
                                          </div>
                                      ...
                                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-3 bg-[#0a0a0f] border border-[rgba(255,255,255,0.05)] rounded-xl">
                                        <div>
                                          ...
                                          <pre className="font-mono text-[10px] text-[var(--text-secondary)] whitespace-pre-wrap leading-relaxed">
                              ...
                              <p className="text-[var(--text-muted)] italic text-xs py-2">Details not available.</p>
                            )}
                          </td>
                        </tr>
                      )}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <button
            ...
            disabled={page === 1}
            className="px-3 py-1.5 rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#0f0f16] text-xs text-[var(--text-secondary)] hover:text-white disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer transition-all"
          >
            Prev
          </button>
